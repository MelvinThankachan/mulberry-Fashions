...
def new_address(request):
    if request.method == "POST":
        ...
        customer = Customer.objects.get(id=request.user.id)
        address_parts = [name, building, street, f"{district}, {state}",pincode, f"Mobile: {mobile}"]
        if city:
...
def edit_address(request, address_id):
    ...
    if request.method == "POST":
        ...
        state = request.POST.get("state").title()
        customer = Customer.objects.get(id=request.user.id)
        address_parts = [name, building, street, f"{district}, {state}", f"Pincode - {pincode}", f"Mobile: {mobile}"]
        if city:
            address_parts.insert(3, city)
        address_text = "\n".join(address_parts)
...
def place_order(request):
    if request.method == "POST":
        ...
        order = Order.objects.create(
            customer=customer, address=address.address_text, total_amount=cart.total_amount
        )
